  ...
  return (
    <div className="p-8 max-w-5xl mx-auto min-h-screen">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Project Helper</h1>
        <p className="text-slate-500">From idea to execution in 4 steps.</p>
      </div>
      ...
      {step === 1 && (
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm max-w-xl mx-auto">
          <h2 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
            <Layers className="text-indigo-600" /> Topic & Subject
          </h2>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">Broad Topic / Interest</label>
              <input 
                type="text" 
                value={topic}
      ...
      {step === 2 && (
        <div className="max-w-3xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-slate-800">Select an Idea</h2>
            <button onClick={() => setStep(1)} className="text-sm text-slate-500 hover:text-indigo-600 flex items-center gap-1"><ArrowLeft size={16} /> Back</button>
          </div>
          ...
          <div className="grid gap-4">
            {ideas.map((idea, idx) => (
              <div 
                ...
                className="bg-white p-6 rounded-2xl border border-slate-200 hover:border-indigo-500 hover:shadow-md cursor-pointer transition-all group"
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 group-hover:text-indigo-700">{idea.title}</h3>
                    <p className="text-slate-500 mt-2 text-sm">{idea.description}</p>
                  </div>
      ...
      {step === 3 && (
        <div className="max-w-3xl mx-auto">
           <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setStep(2)} className="text-sm text-slate-500 hover:text-indigo-600 flex items-center gap-1"><ArrowLeft size={16} /> Back</button>
          </div>

          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm mb-6">
             <h3 className="text-2xl font-bold text-indigo-700 mb-4">{selectedIdea?.title}</h3>
             
             <div className="bg-indigo-50 p-4 rounded-xl mb-6">
                <h4 className="font-semibold text-indigo-900 mb-2">Preliminary Outline</h4>
                <ul className="list-disc list-inside text-indigo-800/80 text-sm space-y-1">
      ...
      {step === 4 && report && (
        <div className="max-w-4xl mx-auto">
           <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-slate-800">Project Report Draft</h2>
            <div className="flex gap-2">
               <button onClick={() => setStep(1)} className="px-4 py-2 text-sm font-medium text-indigo-600 bg-indigo-50 rounded-lg hover:bg-indigo-100">New Project</button>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
             <div className="md:col-span-2 space-y-6">
                <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
                   <h3 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">Process Description</h3>
                   <p className="text-slate-600 leading-relaxed text-sm">{report.processDescription}</p>
                </div>

                <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
                   <h3 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">Step-by-Step Execution</h3>
                   <div className="space-y-4">
             ...
             <div className="space-y-6">
                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                   <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2"><DollarSign size={18} className="text-green-600" /> Estimated Cost</h3>
                   <div className="text-3xl font-bold text-green-600">{report.estimatedCost}</div>
                   <p className="text-xs text-slate-400 mt-1">Approximate market rates</p>
                </div>

                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                   <h3 className="font-bold text-slate-900 mb-4">Required Materials</h3>
                   <ul className="space-y-2">
                      {report.materials.map((m, i) => (
                         <li key={i} className="text-sm text-slate-600 flex items-start gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 mt-1.5"></span> {m}
                         </li>
                      ))}
                   </ul>
                </div>
