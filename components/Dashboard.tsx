...
const Dashboard: React.FC<DashboardProps> = ({ changeView, streak }) => {
  ...
  useEffect(() => {
    const updateDate = () => {
      ...
      setCurrentDate(`Today, ${month} ${day}`);
    };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Hello, {user?.name || 'User'}! Ready to learn something new?
          </p>
        </div>
        <div className="flex items-center gap-4">
           <button className="bg-white dark:bg-slate-800 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm text-sm font-medium text-slate-600 dark:text-slate-300 flex items-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-700">
             <Calendar size={16} /> {currentDate || 'Today, ...'}
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-pink-50 dark:bg-pink-900/20 border border-pink-100 dark:border-pink-800/50 p-6 rounded-3xl flex flex-col justify-between">
          <div className="flex justify-between items-start mb-6">
             <div>
               <h3 className="text-lg font-bold text-pink-900 dark:text-pink-200">Usage This Month</h3>
               <p className="text-pink-700/70 dark:text-pink-200/90 text-sm mt-1">You've been incredibly productive!</p>
             </div>
             <div className="p-2 bg-pink-100 dark:bg-pink-800/50 rounded-lg text-pink-600 dark:text-pink-300">
               <Zap size={20} />
             </div>
          </div>
          
          <div>
            <div className="flex justify-between text-xs font-semibold text-pink-800 dark:text-pink-200 mb-2 uppercase tracking-wide">
               <span>Features Used</span>
               <span>Top 5% of Students</span>
            </div>
            {/* Horizontal Bar */}
            <div className="flex h-4 w-full rounded-full overflow-hidden bg-pink-200/50 dark:bg-pink-800/30">
               <div className="w-[40%] bg-pink-400 dark:bg-pink-500"></div> {/* Chat */}
               <div className="w-[30%] bg-pink-500 dark:bg-pink-600"></div> {/* Tests */}
               <div className="w-[20%] bg-pink-600 dark:bg-pink-700"></div> {/* Project */}
               <div className="w-[10%] bg-pink-300 dark:bg-pink-400"></div> {/* Other */}
            </div>
             <div className="flex gap-4 mt-3 text-xs text-pink-700 dark:text-pink-200">
                <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-pink-400 dark:bg-pink-500"></div> Chatbot</span>
                <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-pink-500 dark:bg-pink-600"></div> Tests</span>
                <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-pink-600 dark:bg-pink-700"></div> Projects</span>
             </div>
          </div>
        </div>
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 p-6 rounded-3xl flex flex-col justify-between">
           <div>
             <h3 className="text-lg font-bold text-emerald-900 dark:text-emerald-200 mb-1">Study Focus</h3>
             <p className="text-emerald-700/70 dark:text-emerald-200/90 text-sm">Suggested daily goal</p>
           </div>
           
           <div className="flex items-end gap-2 mt-4">
              <span className="text-4xl font-bold text-emerald-800 dark:text-emerald-200">4.2</span>
              <span className="text-emerald-600 dark:text-emerald-300 font-medium mb-1">hrs/day</span>
           </div>

           <div className="mt-6 p-4 bg-white/60 dark:bg-emerald-900/30 rounded-2xl border border-emerald-100/50 dark:border-emerald-800/30">
              <div className="flex items-center gap-3">
                 <div className="p-2 bg-emerald-100 dark:bg-emerald-800/50 text-emerald-600 dark:text-emerald-300 rounded-lg">
                    <BookOpen size={18} />
                 </div>
                 <div>
                    <div className="text-xs text-emerald-600 dark:text-emerald-300 font-semibold uppercase">Focus Subject</div>
                    <div className="font-bold text-emerald-900 dark:text-emerald-200">Organic Chemistry</div>
                 </div>
              </div>
           </div>
        </div>
